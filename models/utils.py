...
def calc_ppf(points, point_normals, patches, patch_normals):
    '''
    Calculate ppf gpu
    points: [b, n, 3]
    point_normals: [b, n, 3]
...
class PointNet_SA_Module_KNN_Lite(nn.Module):
    def __init__(self, npoint, nsample, in_channel, out_channel, if_bn=True, group_all=False, use_xyz=True, if_idx=False):
        """
        Args:
            ...
            mlp: list of int,
        """
        super(PointNet_SA_Module_KNN_Lite, self).__init__()
        self.npoint = npoint
        ...
        if use_xyz:
            in_channel += 3
        self.mlp_conv = Conv2d(in_channel, out_channel, if_bn=if_bn)

    def forward(self, xyz, points, ppfs, idx=None):
        """
        Args:
            xyz: Tensor, (B, 3, N)
            points: Tensor, (B, f, N)
        ...
        if self.group_all:
            ...
            new_xyz, new_points, idx, grouped_xyz = sample_and_group_knn_dist(xyz, ppfs, points, self.npoint, self.nsample, self.use_xyz, idx=idx)
...
def square_distance(src, dst):
    """
    Calculate Square of Euclid distance between each two points.
...
def query_knn_and_dist(nsample, xyz, new_xyz, include_self=True):
    """Find k-NN of new_xyz in xyz returns the distances as well"""
    pad = 0 if include_self else 1
    sqrdists = square_distance(new_xyz, xyz)  # B, S, N
    sorted_dists, arg_idx = torch.sort(sqrdists)
    return arg_idx[:, :, pad: nsample+pad].int(), sorted_dists
...
def sample_and_group_knn_dist(xyz, ppfs, points, npoint, k, use_xyz=True, idx=None):
    """
    Args:
        xyz: Tensor, (B, 3, N)
        points: Tensor, (B, f, N)
    ...
    """
    xyz_flipped = xyz.permute(0, 2, 1).contiguous() # (B, N, 3)
    ppf_dim = len(ppfs.shape)

    if ppf_dim == 4:
        ppfs_channel_first = ppfs.permute(0,3,1,2).contiguous() 
    elif ppf_dim == 3:
        ppfs_channel_first = ppfs.permute(0,2,1).contiguous() 
    
    fps_idx = furthest_point_sample(xyz_flipped, npoint)
    new_xyz = gather_operation(xyz, fps_idx) # (B, 3, npoint)
    if idx is None:
        idx, dists = query_knn_and_dist(k, xyz_flipped, new_xyz.permute(0, 2, 1).contiguous())

    grouped_xyz = grouping_operation(xyz, idx) # (B, 3, npoint, nsample)
    #grouped_xyz -= new_xyz.unsqueeze(3).repeat(1, 1, 1, k)

    if points is None:
        new_points = grouping_operation(ppfs_channel_first, idx)
...
class Transformer(nn.Module):
    ...
    def forward(self, x, pos):
        ...
        #pos_rel = torch.pow(pos.reshape((b, -1, n, 1)) - grouping_operation(pos, idx_knn),2)  # b, 3, n, n_knn

        pos_rel = torch.sqrt(  torch.sum(torch.pow(pos.reshape((b, -1, n, 1)) - grouping_operation(pos, idx_knn),2), dim=1, keepdim=True)   )  # b, 3, n, n_knn
        pos_rel = torch.trunc(pos_rel * 1e4) / 1e4
